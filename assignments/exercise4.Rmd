...
1. **HCRIS**: All hospitals that treat Medicare patients must submit a hospital cost report. The data are then made available on the CMS website. The NBER also provides a slightly more user-friendly version of these data. Info and code for these data are available at my GitHub page [here](https://github.com/imccart/HCRIS). The raw data are available on our AWS AMI in the "HCRIS" folder. Use these data for information on hospital beds and to construct a measure of hospital prices.
...
1. Calculate hospital market shares when defining the "market" by zip code and using hospital beds as your measure of quantity. Create a "violin plot" of market shares over time.
